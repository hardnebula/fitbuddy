...
interface UserGroupActivityProps {
  checkIns: CheckIn[];
  userId: string | null;
...
export function UserGroupActivity({
  checkIns,
  userId,
  totalCheckIns,
  bestStreak,
  onCheckInPress,
  onRefresh,
}: UserGroupActivityProps) {
  const { colors, isDark } = useTheme();

  // Filter check-ins to only show the current user's check-ins for stats
  const userCheckIns = useMemo(() => {
    if (!userId) return checkIns.filter((ci) => ci.userId === 'local');
    return checkIns.filter((ci) => String(ci.userId) === String(userId));
  }, [checkIns, userId]);

  // Show all group check-ins (not filtered)
  const allGroupCheckIns = checkIns;
  ...
  return (
    <View style={styles.feedSection}>
      ...
      {/* Stats Cards - showing user's personal stats */}
