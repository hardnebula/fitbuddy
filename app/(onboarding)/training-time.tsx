...
export default function TrainingTimeScreen() {
  ...
  const handleContinue = async () => {
    if (selectedTime) {
      await updateOnboardingData({ trainingTime: selectedTime });
        ...
        <View style={styles.scrollContainer} onLayout={handleLayout}>
          ...
          >
            <View style={styles.content}>
              <Image
                source={require('../../assets/images/Onboarding/Pata Reloj.png')}
                style={styles.illustration}
                resizeMode="contain"
              />
              <Text style={[styles.title, { color: colors.text }]}>When do you usually exercise?</Text>
              <Text style={[styles.subtitle, { color: colors.textSecondary }]}>This helps time your reminders.</Text>

              <View style={styles.chipsContainer}>
                {TIMES.map((time) => (
                  <View key={time} style={styles.chipWrapper}>
                    <SelectionChip
                      label={time}
